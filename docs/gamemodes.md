...
VanillaGamemode registers 12 global data providers during `initialize()` that power the core combat and spell systems:

| Provider | Purpose |
|----------|---------|
| `CombatFormulaProvider` | Attack rolls, defence rolls, hit chance, max hit |
| `WeaponDataProvider` | Weapon categories, attack speeds, combat styles |
| `SpecialAttackProvider` | Special attack definitions |
| `EquipmentBonusProvider` | Equipment stat bonuses, set effects |
| `SpellDataProvider` | Spell definitions, autocast support |
| `SpellXpProvider` | Base XP per spell |
| `RuneDataProvider` | Rune IDs, combination runes, staff substitutions |
| `ProjectileParamsProvider` | Projectile speed, arc, offsets |
| `SkillConfigurationProvider` | Combat level formula, skill decay rates |
| `CombatStyleSequenceProvider` | Melee attack animation selection |
| `SpecialAttackVisualProvider` | Special attack visual overrides |
| `InstantUtilitySpecialProvider` | Rock Knocker, Fishstabber, Lumber Up specials |
