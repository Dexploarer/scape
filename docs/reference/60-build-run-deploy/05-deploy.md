...
browsers  ──HTTPS──►  static host (Vercel / Netlify / Cloudflare Pages / S3 / Caddy)
                     ...
                     XRSPS game server
                            │
                            ├── auth: server/data/accounts.json (or Postgres)
                            └── gameplay: server/data/gamemodes/<id>/player-state.json
```
...
Edit `server/config.json` (or use env vars). Key fields:
...
- **`accountsFilePath`** — path to the JSON auth store. Default is `server/data/accounts.json`.
...
- **Backups** — for local JSON deployments, back up both `server/data/accounts.json` and `server/data/gamemodes/<id>/player-state.json`. If auth uses Postgres, back up the DB plus `player-state.json`.
...
- **Persistence migration** — auth and gameplay state are separate seams. `JsonAccountStore` can be replaced by `PostgresAccountStore`, while gameplay state still flows through `PersistenceProvider` / `PlayerPersistence`. See [20.10](../20-server/10-persistence.md).
...
- **Auth storage default**: `server/data/accounts.json` (JSON file).
- **Gameplay-state default**: `server/data/gamemodes/<id>/player-state.json`.
