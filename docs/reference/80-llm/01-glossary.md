...
| **client plugin** | A client-side feature module under `src/client/plugins/`. Often feeds data into `OverlayManager` or exposes a sidebar panel. Distinct from extrascripts (which are server-side). |
...
| **account store** | Authentication-record storage (usernames, password hashes, bans). Default is `JsonAccountStore`; gameplay state is separate. See [20.10](../20-server/10-persistence.md). |
