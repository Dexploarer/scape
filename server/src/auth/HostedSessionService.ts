...
export class HostedSessionService {
    ...
    verify(
        ...
        if (!trimmed) {
            return { ok: false, code: "missing_session_token", message: "sessionToken is required." };
        }
        const parts = trimmed.split(".");
        if (parts.length !== 3 || parts[0] !== "hs1") {
            return { ok: false, code: "bad_session_token", message: "Malformed hosted session token." };
        }
        ...
        if (
            ...
            return { ok: false, code: "bad_session_signature", message: "Hosted session signature mismatch." };
        }
        ...
        try {
            ...
            return { ok: false, code: "bad_session_payload", message: "Hosted session payload is not valid JSON." };
        }

        if (parsed.version !== 1) {
            return { ok: false, code: "bad_session_version", message: "Unsupported hosted session token version." };
        }
        if (parsed.kind !== expected.kind) {
            return { ok: false, code: "bad_session_kind", message: "Hosted session token kind mismatch." };
        }
        if (expected.worldId && parsed.worldId !== expected.worldId) {
            return { ok: false, code: "bad_session_world", message: "Hosted session world mismatch." };
        }
        if (
            expected.worldCharacterId &&
            parsed.worldCharacterId !== expected.worldCharacterId
        ) {
            return {
                ok: false,
        ...
        if (expected.agentId && parsed.agentId !== expected.agentId) {
            return { ok: false, code: "bad_agent_id", message: "Hosted session agent id mismatch." };
        }

        const now = this.now();
        if (!Number.isFinite(parsed.issuedAt) || !Number.isFinite(parsed.expiresAt)) {
            return { ok: false, code: "bad_session_times", message: "Hosted session timestamps are invalid." };
        }
        if (parsed.expiresAt <= now) {
            return { ok: false, code: "session_expired", message: "Hosted session expired." };
        }
        if (parsed.issuedAt > now + 60_000) {
            return { ok: false, code: "session_from_future", message: "Hosted session issue time is invalid." };
        }
