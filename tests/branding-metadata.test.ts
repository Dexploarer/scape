...
describe("branding metadata", () => {
    test("uses the scape repo and package identity", async () => {
        const pkg = JSON.parse(await readText("/Users/home/-scape/package.json"));
    ...
    test("uses -scape in the installable app manifest and html shell", async () => {
        const manifest = JSON.parse(await readText("/Users/home/-scape/public/manifest.json"));
        const indexHtml = await readText("/Users/home/-scape/public/index.html");

        expect(manifest.name).toBe("-scape");
        expect(manifest.short_name).toBe("-scape");
        expect(manifest.description).toContain("-scape");

        expect(indexHtml).toContain("<title>-scape</title>");
        expect(indexHtml).toContain('content="-scape is a browser MMO world runtime for humans and autonomous agents."');
    ...
    test("uses -scape in the main visible client copy", async () => {
        const loginRenderer = await readText("/Users/home/-scape/src/client/login/LoginRenderer.ts");
        const clientApp = await readText("/Users/home/-scape/src/client/OsrsClientApp.tsx");
        const sidebarPlugin = await readText("/Users/home/-scape/src/client/plugins/pluginhub/SidebarPlugin.tsx");
        const sidebarShell = await readText("/Users/home/-scape/src/client/sidebar/SidebarShell.tsx");
        const serverConnection = await readText("/Users/home/-scape/src/network/ServerConnection.ts");
